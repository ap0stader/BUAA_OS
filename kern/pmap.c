...
void page_init(void) {
	...
	u_long marked_used_page = PPN(PADDR(freemem));
	for (u_long i = 0; i < marked_used_page; i++) {
		pages[i].pp_ref = 1;
	}
	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */
	for (u_long i = marked_used_page; i < npage; i++) {
		pages[i].pp_ref = 0;
